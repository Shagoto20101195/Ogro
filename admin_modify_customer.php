<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST")
    {
        ...
        else
        {
            ...
            if(!empty($address))
            {
                $query = "update customer set address = '$address' where c_id = '$id'";
                mysqli_query($connect, $query);
            }

            if(!empty($count))
            {
                $query = "update customer set purchase_count = '$count' where c_id = '$id'";
                mysqli_query($connect, $query);
            }

            if(!empty($phone))
            {
                $query = "update customer set phone = '$count' where c_id = '$id'";
                mysqli_query($connect, $query);
            }
        }
...
<html>
    <head>
        ...
        <title>Ogro - Modify Customer</title>
    </head>
    ...
    <body>
        ...
        <main>
            <div style="display: block; padding: 300px;">
                <form method="POST">
                    <label for="id"><h1>Enter customer ID</h1></label>
                    ...
                    <label for="phone"><h1>Updated phone number:</h1></label>
                    <input type="text" name="phone" id="phone" size="100%" placeholder="e.g. 2441139" style="padding: 10px;">

                    <br> <br>
                    <br> <br>

                    <label for="count"><h1>Updated purchase count:</h1></label>
